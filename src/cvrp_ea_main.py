"""CVRP EA主循环（支持混合并发：LLM多线程 + 评估多进程）"""
from concurrent.futures import ThreadPoolExecutor, ProcessPoolExecutor, as_completed
from cvrp_operator_population import CVRPOperatorPopulation
from prompt_population import PromptPopulation
from logger import ExperimentLogger
from cvrp_worker import evaluate_one_process
...
class CVRPEAController:
    def __init__(self, instances, log_dir="logs"):
        self.instances = instances
        self.operator_pop = CVRPOperatorPopulation(OPERATOR_POP_SIZE, instances)
    ...
    def _generate_offspring(self, parent_code, prompt_text, top_k_operators, top_k_prompts):
        ...
        system_prompt = "你是CVRP（带容量约束的车辆路径问题）算法专家，只输出Python代码，不要任何解释"
...
【CVRP问题说明】
解的表示：list of lists，每个子列表是一条路线上的客户编号列表（不含depot=0）。
info中包含：demands, capacity, n_customers, progress等。
必须保证：每个客户恰好访问一次，每条路线总需求不超过capacity。
...
2. operator函数：
   ...
   - 可以利用info中的demands, capacity, progress等信息
3. process_info函数：
        ...
        with ProcessPoolExecutor(max_workers=MAX_WORKERS_EVAL) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    print(f"    ⏰ [{idx+1}/{OPERATOR_POP_SIZE}] 进程超时({PROCESS_TIMEOUT}秒)，已强制终止")
                    future.cancel()
                except Exception as e:
