...
import math
import time
from cvrp_problem import generate_initial_solution, calculate_total_cost, check_solution_validity
...
def simulated_annealing(instance, operator_func, process_info_func, accept_criterion_func, 
    ...
    """
    SA框架 for CVRP（支持自定义组件，带超时保护）
    Args:
        instance: CVRP算例
        operator_func: 邻域算子函数 operator(solution, dist_matrix, info)
        process_info_func: 信息加工函数 process_info(iteration, total_iter, T, T_init, current_cost, best_cost, dist_matrix, cost_history)
    ...
    # 初始化
    current = generate_initial_solution(instance.n_customers, instance.demands, instance.capacity)
    ...
    while T > T_end:
        ...
        for _ in range(max_iter):
            ...
            try:
                ...
                # 注入CVRP特有信息
                info['demands'] = instance.demands
                info['capacity'] = instance.capacity
                info['n_customers'] = instance.n_customers
            except Exception:
                info = {
            ...
            try:
                ...
                # 检查解的有效性
                valid, msg = check_solution_validity(
                    neighbor, instance.n_customers, instance.demands, instance.capacity
                )
                if not valid:
                    failed_attempts += 1
                    if failed_attempts > max_failed_attempts:
                ...
                if accept:
                    current = [route[:] for route in neighbor]
